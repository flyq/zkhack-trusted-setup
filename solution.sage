...
# G_1 和 G_2 某个 subgroup 的 order，是个大素数。
...
print(F2)
# Finite Field in u of size 
# 4002409555221667393417789825735904156556882819939007885332058136124031650490837864442687629129015664037894272559787^2

# 定义 G_2
# y^2 = x^3 + 4 * (1 + u) mod (field_modulus + field_modulus * u)
...
# 直接在 G_1 上求解离散对数问题，会很困难，我们需要消去 desired_curve_order

# r*G1 生成的子群的 order 3 * 11 * 10177 * 859267 * 52437899
r_G1 = desired_curve_order * G1
r_s_G1 = desired_curve_order * s_G1
# r_s_G1 肯定在 r_G1 生成的子群中
...
# 使用 sage 内置的 discrete_log() 函数，里面实现了 Pohlig–Hellman 算法
discrete_log(r_s_G1,r_G1,operation='+')
# s_1 = 2335387132884273659
# s = 2335387132884273659 + k_1 * n_1
# 如果我们现在通过暴力搜索 k_1，效果会比较差，因为 n_1 只有 20 位十进制，离 38 位十进制的 s 还有很大的距离。
# 所以我们需要结合 G_2 来求解。
...
# G2.order() 运行很久都得不到结果，因此我们需要手动计算
# G2.order() 
...
factor(G2_order/desired_curve_order)
